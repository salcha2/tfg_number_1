			...
			$.ajax({
    			...
    			success: function(data){
    				...
    				if (check_values_returned (tmp)) {
    					...
    					// Feedback al usuario			   
   					document.getElementById('error_coordinates_messages').innerHTML ="Automatic input variables 1, and 16 to 21, correctly assigned";
    			...
    			error: function(data){
					...
					alert("An error has occurred in the connection: "+data+" " +JSON.stringify(data)); //===Show Error Message====
				},
				// Progress Bar				
 				xhr: function () {
...
function check_coordinates (extent_map) {
	...
	if (lat_user=="" || lon_user=="") {
		document.getElementById('error_coordinates_messages').innerHTML = "Error: you must select or enter some coordinates";
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;
	} else if (isFloat(lat_user)!=1 || isFloat(lon_user)!=1) {
		document.getElementById('error_coordinates_messages').innerHTML = "Error: the coordinates must be numbers with decimals (whole_number_part.fractional_part), and must represent decimal degrees of latitude and longitude (EPSG:4326 system) ";
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;
	} else if (lat_user < extent_map[1] || lat_user > extent_map[3]) {
		 // West, South, East, North
		// var extent = [-20.41080340912, 26.022737926856, 6.58919659088, 45.40273792685601]; 
		document.getElementById('error_coordinates_messages').innerHTML = "Latitude error: there is no data for that latitude. The value must be between "+extent_map[3]+" and "+extent_map[1];
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;
	} else if (lon_user < extent_map[0] || lon_user > extent_map[2]) {
		 // West, South, East, North
		// var extent = [-20.41080340912, 26.022737926856, 6.58919659088, 45.40273792685601]; 
		document.getElementById('error_coordinates_messages').innerHTML = "Longitude error: there is no data for this longitude. The value must be between"+extent_map[0]+" and "+extent_map[2];
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;
	} else {
		lat = lat_user;
...
function check_values_returned (vector) {
	...
	if (!Array.isArray(vector))	{
		...
		document.getElementById('error_coordinates_messages').innerHTML = "Error, received values to be assigned to automatic variables are not a vector: "+vector;
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;	
	} else if (vector.length != 8) {
		// El código PHP al que se le han pedido los datos ha fallado
		document.getElementById('error_coordinates_messages').innerHTML = "Error, not received 7 values for all automatic variables. Received: "+vector.length-1+" "+vector;
		document.getElementById('error_coordinates_messages').style.color='red';
		return 0;
	} else {
		...
		if (cont>0) {
			...
			if (vector[0] == 8) {
				...
				document.getElementById('error_coordinates_messages').innerHTML = "Error: unfortunately the tool does not have geotechnical data for these coordinates";
				document.getElementById('error_coordinates_messages').style.color='red';
			}	else {	
			
			document.getElementById('error_coordinates_messages').innerHTML = "Error: a point at sea has been selected, or a point that is outside Spain";
			document.getElementById('error_coordinates_messages').style.color='red';
